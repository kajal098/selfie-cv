...
class SelfiecvAndroid < Grape::API
  ...
  resources :member do 
      ...
      post :view_another_user_all_stuff , jbuilder: 'android_another_stuff' do
        authenticate!
        @user_stuff = User.find params[:user_id]
        error!({error: 'User not found', status: 'Fail'}, 200) unless @user_stuff      
      end
  ...
  before { authenticate! }
      ...
      post :view, jbuilder: 'android_folder' do
        @my_folder_fav = UserFavourite.where(user_id: current_user.id).where(folder_id: params[:folder_id]).all
  ...
  #--------------------------------folder start----------------------------------#
    ...
    #before { authenticate! }
      ...
      post :search, jbuilder: 'android_notification' do
