class DeviceReport
  ...
  column(:user_id, header: "User") do |model|
    model.user_id ? model.user.username : "Not Available"
  end
  ...
  column(:actions, header: "Action", html: true ) do |user|
    html = link_to "", admin_device_path(user), class: "margin_class btn btn-danger btn-xs glyphicon glyphicon-remove", method: :delete, title: "Remove Device", 'data-confirm' => 'Are you sure?'
    html
  end
